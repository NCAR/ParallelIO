                      ...
                      int *rec_varid, int *norec_varid, int num_types, int *varid_4d)
{
    ...
    for (int t = 0; t < num_types; t++)
    {
        ...
        for (int v = 0; v < NUM_4D_VARS; v++)
        {
            ...
            if (!(data_in_4d = malloc(size * VERT_LEN * LAT_LEN * LON_LEN * NREC)))
                BAIL(PIO_ENOMEM);

            /* Read the data. */
            if ((ret = PIOc_get_var(ncid, varid_4d[v], data_in_4d)))
                BAIL(ret);
            ...
            for (int r = 0; r < LAT_LEN * LON_LEN * NREC; r++)
            {
                ...
                {
                case PIO_INT:
                    if (((int *)data_in_4d)[r] != expected_int_4d[r % (VERT_LEN * LAT_LEN * LON_LEN)])
                        BAIL(ERR_WRONG);
                    break;
                case PIO_FLOAT:
                    if (((float *)data_in_4d)[r] != expected_float_4d[r % (VERT_LEN * LAT_LEN * LON_LEN)])
                        BAIL(ERR_WRONG);
                    break;
                default:
                    BAIL(ERR_WRONG);
