 ...
 */
int test_darray_uneven(int iosysid, int ioid, int num_flavors, int *flavor, int my_rank,
...
#define NUM_TYPES_TO_TEST 3
    ...
    for (int t = 0; t < NUM_TYPES_TO_TEST; t++)
    {
        ...
        if ((ret = test_darray_uneven(iosysid, ioid, num_flavors, flavor, my_rank, pio_type[t], array_len[my_rank])))
            return ret;
