                ...
                PIO_Offset len, nc_type memtype, const void *op)
{
    ...
    if (ios->ioproc)
    {
	    ...
	    signed char* bp;
		    ...
		    bp = (signed char *)op;
		    *sp = *bp;
                    z5writeAttributesshort(all_name, name, (const short*)op);
                    break;
                case NC_USHORT:
...
PIOc_get_att_tc(int ncid, int varid, const char *name, nc_type memtype, void *ip)
{
    ...
    if (ios->ioproc)
    {
		...
		case NC_UINT:
		    ...
		    break;
		case NC_INT:
		    z5readAttributesint(all_name, name, ip);
		    break;
		case NC_FLOAT:
		    z5readAttributesfloat(all_name, name, ip);
		    break;
		case NC_DOUBLE:
		    z5readAttributesdouble(all_name, name, ip);
		    break;
                case NC_SHORT:
		case NC_BYTE:
                    z5readAttributesshort(all_name, name, ip);
                    break;
                case NC_USHORT:
                    z5readAttributesushort(all_name, name, ip);
                    break;
                case NC_INT64:
                    z5readAttributeslonglong(all_name, name, ip);
                    break;
                case NC_UINT64:
                    z5readAttributesulonglong(all_name, name, ip);
                    break;
            default:
                return pio_err(ios, file, PIO_EBADTYPE, __FILE__, __LINE__);
            }
