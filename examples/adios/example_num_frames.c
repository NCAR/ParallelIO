...
int main(int argc, char* argv[])
{
  ...
  PIOc_InitDecomp(iosysid, PIO_INT, 1, dim_len, (PIO_Offset)elements_per_pe, compdof, &wr_iodesc, NULL, NULL, NULL);
...
#if 0
    /* If we define time as unlimited dimension, ADIOS type works */
    PIOc_def_dim(ncid, "time", NC_UNLIMITED, &dimid[0]);
#else
    /* If we define time as limited dimension, ADIOS type fails */
