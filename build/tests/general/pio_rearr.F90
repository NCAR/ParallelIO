...
! from /home/tkurc/codar/e3sm/pio_adios1/ParallelIO/tests/general/pio_rearr.F90.in
...
SUBROUTINE open_and_check_rdwr(comm, iosys, iotype, pio_file, fname, ret)   ! pio_rearr.F90.in:37
    ...
    call PIO_closefile(pio_file)   ! pio_rearr.F90.in:112


    ret = PIO_openfile(iosys, pio_file, iotype, fname, PIO_nowrite)   ! pio_rearr.F90.in:114
    
    IF (.NOT. (PIO_TF_Passert_((ret) == PIO_NOERR, pio_tf_comm_))) THEN
      pio_tf_retval_utest_ = -1
      IF (pio_tf_world_rank_ == 0) THEN
        PRINT *, "PIO_TF: PIO Function failed:",&
           "Could not reopen file " // trim(fname),&
          ":", __FILE__, ":", __LINE__,&
    ...
    ret = PIO_inq_varid(pio_file, var_name, pio_var)   ! pio_rearr.F90.in:117
    
    IF (.NOT. (PIO_TF_Passert_((ret) == PIO_NOERR, pio_tf_comm_))) THEN
      pio_tf_retval_utest_ = -1
      IF (pio_tf_world_rank_ == 0) THEN
        PRINT *, "PIO_TF: PIO Function failed:",&
           "Could not inq var :" // trim(fname),&
          ":", __FILE__, ":", __LINE__,&
          "(pio_rearr.F90.in:118)"
      END IF
      RETURN
    END IF   ! pio_rearr.F90.in:118
#else
    call PIO_syncfile(pio_file)   ! pio_rearr.F90.in:120
#endif


    call PIO_read_darray(pio_file, pio_var, iodesc, rbuf, ret)   ! pio_rearr.F90.in:123
    ...
    IF (.NOT. (PIO_TF_Passert_((ret) == PIO_NOERR,  comm))) THEN
      ...
      IF (pio_tf_tmp_comm_rank_ == 0) THEN
        PRINT *, "PIO_TF: PIO Function failed:",&
          ...
          ":", __FILE__, ":", __LINE__,&
          "(pio_rearr.F90.in:124)"
      END IF
      RETURN
    END IF   ! pio_rearr.F90.in:124
    ...
    IF (.NOT. PIO_TF_Check_val_(rbuf, wbuf)) THEN
      ...
      END IF
      RETURN
    END IF   ! pio_rearr.F90.in:126


    deallocate(rbuf)   ! pio_rearr.F90.in:128


    call PIO_freedecomp(iosys, iodesc)   ! pio_rearr.F90.in:130


    call PIO_closefile(pio_file)   ! pio_rearr.F90.in:132
END SUBROUTINE open_and_check_rdwr   ! pio_rearr.F90.in:133


SUBROUTINE test_rearrs_base
  USE pio_tutil
   ! pio_rearr.F90.in:135
  implicit none   ! pio_rearr.F90.in:136


  character(len=PIO_TF_MAX_STR_LEN), target :: fname = "pio_test_rearrs_base.nc"   ! pio_rearr.F90.in:138
  character(len=PIO_TF_MAX_STR_LEN), parameter :: attname = "filename"   ! pio_rearr.F90.in:139
  character(len=PIO_TF_MAX_STR_LEN), parameter :: dimname = "filename_dim"   ! pio_rearr.F90.in:140
  integer, parameter :: NUM_REARRANGERS = 2   ! pio_rearr.F90.in:141
  integer :: rearrs(NUM_REARRANGERS) = (/pio_rearr_subset,pio_rearr_box/)   ! pio_rearr.F90.in:142
  character(len=PIO_TF_MAX_STR_LEN) :: rearrs_info(NUM_REARRANGERS) = (/"PIO_REARR_SUBSET","PIO_REARR_BOX   "/)   ! pio_rearr.F90.in:143
  integer, dimension(:), allocatable :: iotypes   ! pio_rearr.F90.in:144
  character(len=PIO_TF_MAX_STR_LEN), dimension(:), allocatable :: iotype_descs   ! pio_rearr.F90.in:145
  integer :: i, j, k, num_iotypes = 0   ! pio_rearr.F90.in:146
  type(file_desc_t) :: pio_file   ! pio_rearr.F90.in:147


  type(iosystem_desc_t) :: dup_iosys   ! pio_rearr.F90.in:149
  integer :: dup_comm   ! pio_rearr.F90.in:150
  integer :: dup_comm_rank, dup_comm_sz, dup_iosys_base   ! pio_rearr.F90.in:151
  integer :: ret   ! pio_rearr.F90.in:152


  call PIO_TF_Get_nc_iotypes(iotypes, iotype_descs, num_iotypes)   ! pio_rearr.F90.in:154
  do i=1,num_iotypes   ! pio_rearr.F90.in:155
    ...
    call create_file(pio_tf_comm_, pio_tf_iosystem_, iotypes(i), &
                      fname, attname, dimname, ret)   ! pio_rearr.F90.in:158
    ...
    IF (.NOT. (PIO_TF_Passert_((ret) == PIO_NOERR,  pio_tf_comm_))) THEN
      ...
      IF (pio_tf_tmp_comm_rank_ == 0) THEN
        PRINT *, "PIO_TF: PIO Function failed:",&
          ...
          ":", __FILE__, ":", __LINE__,&
          "(pio_rearr.F90.in:159)"
      END IF
      RETURN
    END IF   ! pio_rearr.F90.in:159


    call MPI_Comm_dup(pio_tf_comm_, dup_comm, ret)   ! pio_rearr.F90.in:161
    call MPI_Comm_rank(dup_comm, dup_comm_rank, ret)   ! pio_rearr.F90.in:162
    call MPI_Comm_size(dup_comm, dup_comm_sz, ret)   ! pio_rearr.F90.in:163
    do j=1, NUM_REARRANGERS   ! pio_rearr.F90.in:164
      ...
      IF (pio_tf_world_rank_ == 0) THEN
        ...
        END IF
      END IF   ! pio_rearr.F90.in:165
      do k=1, dup_comm_sz   ! pio_rearr.F90.in:166
        dup_iosys_base = k-1   ! pio_rearr.F90.in:167
        ...
        call PIO_init(pio_tf_world_rank_, dup_comm, pio_tf_world_sz_, &
                      1, &! Num aggregators
                      1, &! Stride
                      rearrs(j), dup_iosys, base=dup_iosys_base)   ! pio_rearr.F90.in:172
        call PIO_seterrorhandling(dup_iosys, PIO_BCAST_ERROR)   ! pio_rearr.F90.in:173


        call open_and_check_rdwr(dup_comm, dup_iosys, iotypes(i), &
                        pio_file, fname, ret)   ! pio_rearr.F90.in:176
        ...
        IF (.NOT. (PIO_TF_Passert_((ret) == PIO_NOERR,  dup_comm))) THEN
          ...
          IF (pio_tf_tmp_comm_rank_ == 0) THEN
            PRINT *, "PIO_TF: PIO Function failed:",&
              ...
              ":", __FILE__, ":", __LINE__,&
              "(pio_rearr.F90.in:177)"
          END IF
          RETURN
        END IF   ! pio_rearr.F90.in:177


        call PIO_finalize(dup_iosys, ret)   ! pio_rearr.F90.in:179
      end do   ! pio_rearr.F90.in:180
    end do   ! pio_rearr.F90.in:181
    call PIO_deletefile(pio_tf_iosystem_, fname)   ! pio_rearr.F90.in:182
    call MPI_Comm_free(dup_comm, ret)   ! pio_rearr.F90.in:183
  end do   ! pio_rearr.F90.in:184
  if(allocated(iotypes)) then   ! pio_rearr.F90.in:185
    deallocate(iotypes)   ! pio_rearr.F90.in:186
    deallocate(iotype_descs)   ! pio_rearr.F90.in:187
  end if   ! pio_rearr.F90.in:188
END SUBROUTINE test_rearrs_base   ! pio_rearr.F90.in:189
...
SUBROUTINE test_rearrs_combs
  USE pio_tutil
   ! pio_rearr.F90.in:195
  implicit none   ! pio_rearr.F90.in:196


  character(len=PIO_TF_MAX_STR_LEN), target :: fname = "pio_rearrs_combs.nc"   ! pio_rearr.F90.in:198
  character(len=PIO_TF_MAX_STR_LEN), parameter :: attname = "filename"   ! pio_rearr.F90.in:199
  character(len=PIO_TF_MAX_STR_LEN), parameter :: dimname = "filename_dim"   ! pio_rearr.F90.in:200
  integer, parameter :: NUM_REARRANGERS = 2   ! pio_rearr.F90.in:201
  integer :: rearrs(NUM_REARRANGERS) = (/pio_rearr_subset,pio_rearr_box/)   ! pio_rearr.F90.in:202
  integer, parameter :: MAX_PERMS = 4   ! pio_rearr.F90.in:203
  ...
  character(len=PIO_TF_MAX_STR_LEN) :: rearrs_perms_info(NUM_REARRANGERS,MAX_PERMS) =&
            reshape(&
              ...
              "PIO_REARR_BOX   ", "PIO_REARR_BOX   "/),&
              (/NUM_REARRANGERS,MAX_PERMS/)&
            )   ! pio_rearr.F90.in:218
  character(len=PIO_TF_MAX_STR_LEN) :: rearrs_info(NUM_REARRANGERS) = (/"PIO_REARR_SUBSET","PIO_REARR_BOX   "/)   ! pio_rearr.F90.in:219
  integer, dimension(:), allocatable :: iotypes   ! pio_rearr.F90.in:220
  character(len=PIO_TF_MAX_STR_LEN), dimension(:), allocatable :: iotype_descs   ! pio_rearr.F90.in:221
  integer :: i, j, k, num_iotypes = 0   ! pio_rearr.F90.in:222
  type(file_desc_t) :: pio_file   ! pio_rearr.F90.in:223


  type(iosystem_desc_t) :: dup_iosys   ! pio_rearr.F90.in:225
  integer :: dup_comm   ! pio_rearr.F90.in:226
  integer :: ret   ! pio_rearr.F90.in:227


  call PIO_TF_Get_nc_iotypes(iotypes, iotype_descs, num_iotypes)   ! pio_rearr.F90.in:229
  do i=1,num_iotypes   ! pio_rearr.F90.in:230
    ...
    IF (pio_tf_world_rank_ == 0) THEN
      IF (pio_tf_log_level_ >= 0) THEN
        ...
        WRITE(*,*)  "Testing iotype: ", iotype_descs(i)
      END IF
    END IF   ! pio_rearr.F90.in:231
    call create_file(pio_tf_comm_, pio_tf_iosystem_, iotypes(i), &
                      fname, attname, dimname, ret)   ! pio_rearr.F90.in:233
    ...
    IF (.NOT. (PIO_TF_Passert_((ret) == PIO_NOERR,  pio_tf_comm_))) THEN
      ...
      IF (pio_tf_tmp_comm_rank_ == 0) THEN
        PRINT *, "PIO_TF: PIO Function failed:",&
           "Failed to create file :" // fname,&
          ":", __FILE__, ":", __LINE__,&
          "(pio_rearr.F90.in:234)"
      END IF
      RETURN
    END IF   ! pio_rearr.F90.in:234


    call MPI_Comm_dup(pio_tf_comm_, dup_comm, ret)   ! pio_rearr.F90.in:236
    ! Try different combinations of rearrangers
    do k=1,MAX_PERMS   ! pio_rearr.F90.in:238
      do j=1, NUM_REARRANGERS   ! pio_rearr.F90.in:239
        ...
        IF (pio_tf_world_rank_ == 0) THEN
          ...
          END IF
        END IF   ! pio_rearr.F90.in:240
        call PIO_init(pio_tf_world_rank_, dup_comm, pio_tf_world_sz_, &
                      1, &! Num aggregators
                      1, &! Stride
                      rearrs_perms(j,k), dup_iosys, base=0)   ! pio_rearr.F90.in:244
        call PIO_seterrorhandling(dup_iosys, PIO_BCAST_ERROR)   ! pio_rearr.F90.in:245


        call open_and_check_rdwr(dup_comm, dup_iosys, iotypes(i), &
                        pio_file, fname, ret)   ! pio_rearr.F90.in:248
        ...
        IF (.NOT. (PIO_TF_Passert_((ret) == PIO_NOERR,  dup_comm))) THEN
          ...
          IF (pio_tf_tmp_comm_rank_ == 0) THEN
            PRINT *, "PIO_TF: PIO Function failed:",&
               "Checking rd+wr on file failed :" // fname,&
              ":", __FILE__, ":", __LINE__,&
              "(pio_rearr.F90.in:249)"
          END IF
          RETURN
        END IF   ! pio_rearr.F90.in:249


        call PIO_finalize(dup_iosys, ret)   ! pio_rearr.F90.in:251
      end do   ! pio_rearr.F90.in:252
    end do   ! pio_rearr.F90.in:253
    call PIO_deletefile(pio_tf_iosystem_, fname)   ! pio_rearr.F90.in:254
    call MPI_Comm_free(dup_comm, ret)   ! pio_rearr.F90.in:255
  end do   ! pio_rearr.F90.in:256
  if(allocated(iotypes)) then   ! pio_rearr.F90.in:257
    deallocate(iotypes)   ! pio_rearr.F90.in:258
    deallocate(iotype_descs)   ! pio_rearr.F90.in:259
  end if   ! pio_rearr.F90.in:260
END SUBROUTINE test_rearrs_combs   ! pio_rearr.F90.in:261
