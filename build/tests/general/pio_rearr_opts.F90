...
! from /home/tkurc/codar/e3sm/pio_adios1/ParallelIO/tests/general/pio_rearr_opts.F90.in
                ...
                do cur_enable_hs_i2c=1,num_enable_hs_opts_io2comp   ! pio_rearr_opts.F90.in:408
                  ...
                  do cur_enable_isend_i2c=1,num_enable_isend_opts_io2comp   ! pio_rearr_opts.F90.in:412
                    ...
                    do cur_max_pend_req_i2c=1,num_max_pend_req_opts_io2comp   ! pio_rearr_opts.F90.in:416
                      ...
                      call PIO_closefile(pio_file)   ! pio_rearr_opts.F90.in:446


                      call open_file_and_get_var(dup_iosys, pio_file, iotypes(i),&
                              pio_var, dims, ret)   ! pio_rearr_opts.F90.in:449
...
#else
                      call PIO_syncfile(pio_file)   ! pio_rearr_opts.F90.in:452
#endif


                      call PIO_read_darray(pio_file, pio_var, iodesc, rbuf, ret)   ! pio_rearr_opts.F90.in:455
                      ...
                      IF (.NOT. (PIO_TF_Passert_((ret) == PIO_NOERR,  dup_comm))) THEN
                        ...
                        IF (pio_tf_tmp_comm_rank_ == 0) THEN
                          PRINT *, "PIO_TF: PIO Function failed:",&
                            ...
                            ":", __FILE__, ":", __LINE__,&
                            "(pio_rearr_opts.F90.in:456)"
                        END IF
                        RETURN
                      END IF   ! pio_rearr_opts.F90.in:456
                      ...
                      call PIO_freedecomp(dup_iosys, iodesc)   ! pio_rearr_opts.F90.in:460


                      deallocate(rbuf)   ! pio_rearr_opts.F90.in:462
                      if(allocated(wbuf)) then   ! pio_rearr_opts.F90.in:463
                        deallocate(wbuf)   ! pio_rearr_opts.F90.in:464
                      end if   ! pio_rearr_opts.F90.in:465


                      call PIO_closefile(pio_file)   ! pio_rearr_opts.F90.in:467


                      call PIO_finalize(dup_iosys, ret)   ! pio_rearr_opts.F90.in:469
                      ...
                      IF (.NOT. (PIO_TF_Passert_((ret) == PIO_NOERR, pio_tf_comm_))) THEN
                        ...
                        IF (pio_tf_world_rank_ == 0) THEN
                          PRINT *, "PIO_TF: PIO Function failed:",&
                             "Finalize failed",&
                            ":", __FILE__, ":", __LINE__,&
                            "(pio_rearr_opts.F90.in:470)"
                        END IF
                        RETURN
                      END IF   ! pio_rearr_opts.F90.in:470
                    end do ! cur_max_pend_req_i2c   ! pio_rearr_opts.F90.in:471
                  end do ! cur_enable_isend_i2c   ! pio_rearr_opts.F90.in:472
                end do ! cur_enable_hs_i2c   ! pio_rearr_opts.F90.in:473
              end do ! cur_max_pend_req_c2i       ! pio_rearr_opts.F90.in:474
            end do ! cur_enable_isend_c2i   ! pio_rearr_opts.F90.in:475
          end do ! cur_enable_hs_c2i   ! pio_rearr_opts.F90.in:476
        end do ! cur_fcd_opt   ! pio_rearr_opts.F90.in:477
      end do ! cur_comm_type_opt   ! pio_rearr_opts.F90.in:478
    end do ! cur_rearr   ! pio_rearr_opts.F90.in:479


    call PIO_deletefile(pio_tf_iosystem_, trim(tgv_fname))   ! pio_rearr_opts.F90.in:481


    call MPI_Comm_free(dup_comm, ret)   ! pio_rearr_opts.F90.in:483
  end do ! iotypes   ! pio_rearr_opts.F90.in:484


  if(allocated(iotypes)) then   ! pio_rearr_opts.F90.in:486
    deallocate(iotypes)   ! pio_rearr_opts.F90.in:487
    deallocate(iotype_descs)   ! pio_rearr_opts.F90.in:488
  end if   ! pio_rearr_opts.F90.in:489


END SUBROUTINE write_with_rearr_opts   ! pio_rearr_opts.F90.in:491
